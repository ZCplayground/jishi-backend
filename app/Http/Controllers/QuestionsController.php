...
class QuestionsController extends Controller
{
    public function questionRequest(Request $request) //请求问题
    {
        ...
        else if($ret === 'success')
        {
            ...
            shuffle($front_two);  //利用shuffle()函数将产生的$num数组随机打乱顺序

            $first_qnum = rand(1,Question::where('attr', $front_two[0])->count()); //获取维度问题数目
            $first_question = Question::where('attr',$front_two[0])->select('id','content')->offset($first_qnum-1)->limit(1)->first();

            $second_qnum = rand(1,Question::where('attr', $front_two[1])->count()); //获取维度问题数目
            ...
            $third_qnum = rand(1,Question::where('attr', $third)->count()); //获取维度问题数目
            ...
            // 返回question_id_list,三个问题,http 200
            return response()->json([
